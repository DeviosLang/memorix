...
against a live memorix-server instance using the `user_space_test_01` database
with auto-embedding enabled.

## Prerequisites

- memorix-server running (default `127.0.0.1:18081`, override with `MNEMO_TEST_BASE`)
...
| `crdt-server-merge-e2e.py` | Server-side section merge: two agents write disjoint sections concurrently, server merges atomically via `X-Memorix-Merged`, both agents read identical final content (13 tests) |
...
- All scripts send `X-Memorix-Agent-Id` header on every request.
