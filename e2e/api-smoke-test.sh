#!/bin/bash
# api-smoke-test.sh
# Smoke test against https://api.memorix.ai (or any mnemo-server instance).
#
# Tests covered:
#   1. Healthcheck
...
#   MNEMO_BASE=https://api.memorix.ai bash e2e/api-smoke-test.sh
set -euo pipefail

BASE="${MNEMO_BASE:-https://api.memorix.ai}"
...
# ============================================================================
step "2" "Provision tenant (POST /v1alpha1/memorix)"
resp=$(curl_json -X POST "$BASE/v1alpha1/memorix")
code=$(http_code "$resp")
bdy=$(body "$resp")
check "POST /v1alpha1/memorix returns 201" "$code" "201"
...
MEM_BASE="$BASE/v1alpha1/memorix/$TENANT_ID/memories"
