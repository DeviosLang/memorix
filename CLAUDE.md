# CLAUDE.md — Agent context for memorix

## What is this repo?

memorix is cloud-persistent memory for AI agents. Plugins connect to memorix-server (Go) for multi-agent, tenant-isolated memory with hybrid vector + keyword search.
...
```bash
# Build server
cd server && go build ./cmd/memorix-server
...
```
server/cmd/memorix-server/main.go     — Entry point, DI wiring, graceful shutdown
...
openclaw-plugin/server-backend.ts   — Server mode: fetch → memorix API
...
- **Server-only architecture**: All plugins connect to memorix-server via REST API. No direct DB access from plugins.
...
```bash
curl -s -X POST http://<server>/v1alpha1/memorix | jq .
...
```
POST   /v1alpha1/memorix/{tenantID}/memories          — Create
GET    /v1alpha1/memorix/{tenantID}/memories           — Search/list
GET    /v1alpha1/memorix/{tenantID}/memories/{id}      — Get by ID
PUT    /v1alpha1/memorix/{tenantID}/memories/{id}      — Update
DELETE /v1alpha1/memorix/{tenantID}/memories/{id}      — Delete
```

Agent identity is passed via the `X-Memorix-Agent-Id` header.
...
1. In Claude Code: `/plugin marketplace add devioslang/memorix`
2. Then: `/plugin install memorix-memory@memorix`
...
If a user asks to install memorix for OpenCode, read `opencode-plugin/README.md` for the full guide.

**npm plugin (recommended):**

1. Add to `opencode.json`: `{"plugin": ["memorix-opencode"]}`
2. Set env vars: `MNEMO_API_URL` and `MNEMO_TENANT_ID`
3. Restart OpenCode — plugin auto-installs from npm and logs `[memorix] Server mode...`

**From source (alternative):**

1. `cd opencode-plugin && npm install`
2. Add to `opencode.json`: `{"plugins": {"memorix": {"path": "/absolute/path/to/memorix/opencode-plugin"}}}`
3. Set env vars (same as above)

## Installing the OpenClaw plugin

If a user asks to install memorix for OpenClaw, read `openclaw-plugin/README.md` for the full guide. Quick summary:

1. `cd openclaw-plugin && npm install`
2. Add to `openclaw.json`:
   - Set `plugins.slots.memory` to `"memorix"`
   - Add `plugins.entries.memorix` with `enabled: true` and config
