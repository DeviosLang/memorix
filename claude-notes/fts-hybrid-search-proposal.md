...
all memorix hybrid search implementations, while retaining `LIKE` as a runtime
...
memorix hybrid search currently runs two queries and merges results client-side:
...
```
openclaw ServerBackend.search()  ->  GET /api/memories?q=...  ->  memorix-server
opencode ServerBackend.search()  ->  GET /api/memories?q=...  ->  memorix-server
claude   mnemo_server_get()       ->  GET /api/memories?q=...  ->  memorix-server
```
...
- **Fail-fast is NOT used**: a cluster without FTS support should still serve
  keyword or vector searches — memorix must not refuse to start. The degraded
...
hybrid_result=$(MNEMO_Q="$query" \
                ...
                MNEMO_DB="${MNEMO_DB_NAME:-memorix}" \
...
"[memorix] Direct mode (auto-embed hybrid: <model>)"
// no vector leg:
"[memorix] Direct mode (keyword search: FTS preferred, LIKE fallback)"
```
