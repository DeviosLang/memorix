...
Upgrade memorix from naive LWW (integer `version++`) to vector-clock-based CRDT conflict resolution with tombstone deletion. This enables true multi-agent concurrency detection: the server can distinguish "A happened before B" from "A and B are concurrent" and resolve conflicts deterministically.

## Context

**Why CRDT now, not later:** Multi-agent concurrent access is the primary design intent of memorix — not a speculative future use case. The architecture explicitly provisions multiple agents per space (`POST /api/spaces/:id/tokens`), and the v2 memo (`claw-memory-v2-memo.md §1`) names silent concurrent overwrite as a *fundamental problem*, not an edge case. CRDT is the correct tool for the stated problem: detecting and resolving concurrent writes without real-time coordination. Deferring it would mean shipping a multi-agent system with known silent data-loss semantics.

The current memorix server (Phase 1) works for single-agent or low-contention multi-agent use. But when multiple agents write the same key concurrently, the server blindly overwrites with the latest request (physical clock ordering from MySQL `ON UPDATE CURRENT_TIMESTAMP`). There is no way to:
...
- CRDT in direct-mode plugin (`DirectBackend` does raw SQL to TiDB Serverless without memorix-server; CRDT logic lives in the server only. Direct-mode continues with simple LWW.)
...
| `POST` (with clock) | any | live, incoming dominates | UPDATE → 201 + `X-Memorix-Winner` |
| `POST` (with clock) | any | live, incoming dominated | no-op → 200 + `X-Memorix-Dominated: true` |
...
| `POST` (with clock) | any | tombstoned, tombstone wins | no-op → 200 + `X-Memorix-Dominated: true` |
...
- If the row has since been modified by a later write (i.e., `last_write_id` no longer matches): the idempotency window has expired. Return the current row state with HTTP 200 and `X-Memorix-Idempotency-Expired: true`. Clients MUST NOT assume the original write result is recoverable after the idempotency window expires.
...
```
X-Memorix-Winner:    agent-a        (origin_agent of the winner; present when merge occurred)
X-Memorix-Dominated: true           (present and "true" when the incoming write was discarded)
```
...
Response logic:
- Clock-aware write, incoming wins (new or dominating): HTTP 201, body = flat `Memory`, set `X-Memorix-Winner: <origin_agent>`.
- Clock-aware write, dominated (existing wins): HTTP 200, body = existing flat `Memory`, set `X-Memorix-Winner: <origin_agent>`, `X-Memorix-Dominated: true`.
...
The plugin now has a dual-mode architecture: `DirectBackend` (raw SQL to TiDB Serverless) and `ServerBackend` (HTTP to memorix-server), abstracted behind the `MemoryBackend` interface.
...
- Optionally reads `X-Memorix-Dominated: true` header if the caller needs to know whether the write was discarded.
...
- Return flat `Memory` from handler; set `X-Memorix-Winner`/`X-Memorix-Dominated` headers
...
- Optionally surface `X-Memorix-Dominated` header in store result
...
3. **Winner value on dominated write** — **Decision: dominated write returns HTTP 200 (not 201), body is the existing winning `Memory` (flat, same shape as GET).** HTTP 201 means "resource created or updated". HTTP 200 means "request processed, here is the current state". Callers that check status can distinguish; callers that ignore status get the current content either way — no silent data loss. The `X-Memorix-Dominated: true` response header signals the no-op without changing the body shape.
