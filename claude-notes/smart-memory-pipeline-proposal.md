...
**Purpose**: Design the intelligent auto-capture pipeline for memorix — extracting atomic facts, generating session digests, and optimizing recall accuracy through a two-phase LLM pipeline.  
...
memorix currently stores memories as raw text blobs via `agent_end` hook. This leads to:
...
agent_end fires with messages
        │
        ▼
Plugin formats & POSTs to memorix-server
...
1. **Server-side intelligence**: Plugins are thin transport. All extraction/reconciliation logic lives in memorix-server.
...
- **Connection pool**: How memorix-server manages per-tenant `*sql.DB` connections
