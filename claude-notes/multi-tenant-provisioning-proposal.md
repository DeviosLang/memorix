...
**Date**: 2026-03-06  
**Purpose**: Design the tenant isolation layer for memorix-server — each OpenClaw instance gets a dedicated TiDB Serverless cluster, with token-based authentication and automatic provisioning.
...
memorix-server operates as a **two-plane** system:

```
┌──────────────────────────────────────────────────────────────┐
│                        CONTROL PLANE                          │
│         memorix-server's own DB (MNEMO_DSN)                     │
│                                                               │
│  ┌─────────────┐  ┌─────────────┐  ┌──────────────────────┐  │
...
### 3.1 Registration: OpenClaw → memorix-server → TiDB Zero
...
OpenClaw agent_end fires
    ...
    │
    ▼
memorix-server auth middleware
    │
