...
memorix uses a custom vector clock CRDT (~70 LoC) for multi-agent conflict resolution. No external library — just three functions in `service/vclock.go`.
...
| Existing dominates | No-op, return existing | 200 + `X-Memorix-Dominated: true` |
