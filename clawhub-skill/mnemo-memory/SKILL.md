...
name: memorix-memory
version: 0.3.0
description: "Cloud-persistent memory for AI agents. Stateless plugins + memorix-server = cross-session recall, multi-agent sharing, and hybrid vector + keyword search. Works with OpenClaw, Claude Code, and OpenCode."
author: devioslang
...
# memorix — Cloud-Persistent Memory for AI Agents 🧠

**Your agents are stateless. Your memory shouldn't be.**

Every AI agent session starts from zero. Context is lost, decisions are forgotten, and your agents keep rediscovering what they already knew. memorix externalizes agent memory into a central server — so agents stay disposable, but memory persists forever.
...
| Pain Point | Without memorix | With memorix |
|---|---|---|
...
| Platform | Install |
|---|---|
| **Claude Code** | `/plugin marketplace add devioslang/memorix` → `/plugin install memorix-memory@memorix` |
| **OpenCode** | `"plugin": ["memorix-opencode"]` in `opencode.json` |
| **Any HTTP client** | REST API via memorix-server |
...
- **GitHub**: [github.com/devioslang/memorix](https://github.com/devioslang/memorix)
- **Design Doc**: [docs/DESIGN.md](https://github.com/devioslang/memorix/blob/main/docs/DESIGN.md)
- **TiDB Cloud**: [tidbcloud.com](https://tidbcloud.com) (free tier)

---

*Built for agents that need to remember. Powered by [memorix-server](https://github.com/devioslang/memorix/tree/main/server).*
