...
export class ServerBackend implements MemoryBackend {
  ...
  async register(): Promise<ProvisionMemorixResponse> {
    const resp = await fetch(this.baseUrl + "/v1alpha1/memorix", {
      method: "POST",
    ...
    if (!resp.ok) {
      ...
      throw new Error(`memorix provision failed (${resp.status}): ${body}`);
    }

    const data = (await resp.json()) as ProvisionMemorixResponse;
    if (!data?.id) {
      throw new Error("memorix provision did not return tenant ID");
    }
  ...
  private async requestRaw(
    ...
    const headers: Record<string, string> = {
      ...
      "X-Memorix-Agent-Id": this.agentName,
    };
    return fetch(url, {
      method,
      headers,
