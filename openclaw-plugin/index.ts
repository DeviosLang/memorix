...
const DEFAULT_API_URL = "https://api.memorix.ai";
...
const mnemoPlugin = {
  ...
  name: "Mnemo Memory",
  description:
    "AI agent memory — server mode (memorix-server) with hybrid vector + keyword search.",

  async register(api: OpenClawPluginApi) {
    const cfg = (api.pluginConfig ?? {}) as PluginConfig;
    const effectiveApiUrl = cfg.apiUrl ?? DEFAULT_API_URL;
    if (!cfg.apiUrl) {
      api.logger.info(`[memorix] apiUrl not configured, using default ${DEFAULT_API_URL}`);
    }
    ...
    const registerTenant = async (agentName: string): Promise<string> => {
      ...
      api.logger.info(
        `[memorix] *** Auto-provisioned tenant_id=${result.id} *** Save this tenant ID to your config as tenantID`
      );
      return result.id;
    };
    ...
    api.logger.info("[memorix] Server mode (tenant-scoped memorix API)");
