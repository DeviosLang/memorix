# OpenClaw Plugin for memorix

Memory plugin for [OpenClaw](https://github.com/openclaw) — replaces the built-in memory slot with cloud-persistent shared memory. Runs in server mode only, connecting to `memorix-server` via `apiUrl` + `tenantID`.

## 🚀 Quick Start (Server Mode)

**You need a running `memorix-server` instance.**
...
curl -s -X POST http://localhost:8080/v1alpha1/memorix \
...
Add memorix to your project's `openclaw.json`:
...
All memory calls use `/v1alpha1/memorix/{tenantID}/memories/...` — the tenant ID is carried in the URL path, and no special headers are required.
...
```bash
openclaw plugins install @memorix/openclaw
```
...
- **Per-agent identity** — every OpenClaw instance can pass its own `X-Memorix-Agent-Id` header
- **Centralized management** — one memorix-server manages all memory, with rate limiting and access controls
...
```bash
cd memorix/server
MNEMO_DSN="user:pass@tcp(tidb-host:4000)/memorix?parseTime=true" go run ./cmd/memorix-server
```
...
If you see `[memorix] No mode configured...`, check your `openclaw.json` config.
...
|---|---|---|
| `apiUrl` | string | memorix-server URL |
| `tenantID` | string | Tenant ID used in `/v1alpha1/memorix/{tenantID}/memories` (preferred) |
...
├── package.json           # npm package (@memorix/openclaw)
