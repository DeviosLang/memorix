/**
 * Lifecycle hooks for the memorix OpenClaw plugin.
 *
...
export function registerHooks(
  ...
  api.on(
    ...
    async (event: unknown) => {
      try {
        ...
        logger.info(`[memorix] Injecting ${memories.length} memories into prompt context`);
        ...
        logger.error(`[memorix] before_prompt_build failed: ${String(err)}`);
      }
    },
  ...
  api.on("after_compaction", async (_event: unknown) => {
    logger.info("[memorix] Compaction detected — memories will be re-queried on next prompt");
  });
  ...
  api.on("before_reset", async (event: unknown) => {
    try {
      ...
      logger.info("[memorix] Session context saved before reset");
    } catch (err) {
      // Best-effort — never block /reset
      logger.error(`[memorix] before_reset save failed: ${String(err)}`);
    }
  });
  ...
  api.on("agent_end", async (event: unknown) => {
    try {
      ...
      if (result.status === "accepted") {
        logger.info("[memorix] Ingest accepted for async processing");
      } else if ((result.memories_changed ?? 0) > 0) {
        logger.info(
          `[memorix] Ingested session: memories_changed=${result.memories_changed}, status=${result.status}`
        );
      }
    } catch {
