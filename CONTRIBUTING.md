...
cd memorix/server
go mod download
go build ./cmd/memorix-server

# Apply schema
mysql -h <host> -P <port> -u <user> -p < schema.sql

# Run
export MNEMO_DSN="user:pass@tcp(host:port)/memorix?parseTime=true"
...
This section is for the OpenClaw integration specifically; memorix supports multiple agent platforms.
...
Please use [GitHub Issues](https://github.com/devioslang/memorix/issues) with:
- Steps to reproduce
